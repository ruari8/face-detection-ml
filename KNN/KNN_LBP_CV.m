...
%% Convert images to LBP feature vectors
[trainFeatures, ~] = lbp_convert_datasets(trainImages, []);
...
%% Display Results
fprintf('\nResults for different k values (Cross-Validation):\n');
...
disp('Cross-validation for KNN with LBP completed.');

%% Cross-Validation Evaluation
fprintf('\nEvaluating KNN classifier with Cross-Validation...\n');
...
% Identify the best k value based on mean accuracy
[~, bestIdx] = max([k_results.mean_accuracy]);
bestK = k_results(bestIdx).k;
fprintf('\nBest k value: %d (Mean Accuracy: %.2f%% ± %.2f%%)\n', ...
    bestK, k_results(bestIdx).mean_accuracy, k_results(bestIdx).std_accuracy);

%% Confusion Matrix and ROC Curve for Best k Value
fprintf('\nFinal evaluation using best k = %d...\n', bestK);
...
% Evaluate on the full training set (or optionally use a separate test set)
...
fprintf('\nPerformance Metrics for Best k = %d:\n', bestK);
fprintf('Accuracy: %.2f%%\n', accuracy);
...
title(sprintf('Confusion Matrix for KNN (k = %d)', bestK));

% Plot ROC Curve and Calculate AUC if Scores are Available
if size(scores, 2) > 1
    fprintf('Plotting ROC curve and calculating AUC...\n');
    [rocX, rocY, ~, AUC] = perfcurve(trainLabels, scores(:, 2), 1); % Assuming scores(:, 2) corresponds to positive class
    ...
    title(sprintf('ROC Curve for KNN (k = %d)', bestK));
...
disp('Cross-Validation Evaluation for KNN completed.');
