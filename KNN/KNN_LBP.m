...
% Normalize the testing images to [0,1] range
testImages = testImages ./ 255.0;

%% Convert images to LBP feature vectors
[trainFeatures, testFeatures] = lbp_convert_datasets(trainImages, testImages);
...
[~, bestIdx] = max([k_results.accuracy]);
bestK = k_results(bestIdx).k;
fprintf('\nBest k value: %d (Accuracy: %.2f%%)\n', bestK, k_results(bestIdx).accuracy);

%% Evaluation
fprintf('Evaluating the KNN classifier...\n');
...
% Display Metrics
fprintf('Performance Metrics for KNN with LBP:\n');
fprintf('Accuracy: %.2f%%\n', accuracy);
...
title('Confusion Matrix for KNN Classifier with LBP');

% Calculate and Plot ROC Curve if Scores are Available
if size(scores, 2) > 1
    ...
    title('ROC Curve for KNN Classifier with LBP');
...
disp('KNN with LBP evaluation completed.');
