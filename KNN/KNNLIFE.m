...
% Normalize the test images to [0,1] range
testImages = testImages ./ 255.0;

% Convert images to LBP feature vectors
[~, testFeatures] = lbp_convert_datasets([], testImages);
...
for k_idx = 1:num_k
    ...
    k_results(k_idx).mean_auc = nanmean(aucs);
    k_results(k_idx).std_auc = nanstd(aucs);
end
...
fprintf('\nEvaluating KNN classifier on test data with k = %d...\n', bestK);

% Train the model using the best k on the entire training set
knnModel = fitcknn(trainFeatures, trainLabels, ...
                   'NumNeighbors', bestK, ...
                   'Distance', 'euclidean');

% Predict on the test data
[predictedTestLabels, testScores] = predict(knnModel, testFeatures);
...
% Test ROC Curve
if size(testScores, 2) > 1
    [rocX, rocY, ~, testAUC] = perfcurve(testLabels, testScores(:, 2), 1);
    figure;
    plot(rocX, rocY, 'LineWidth', 2);
    xlabel('False Positive Rate');
    ylabel('True Positive Rate');
    title(sprintf('ROC Curve for Test Data (k = %d)', bestK));
    legend(sprintf('AUC = %.2f', testAUC), 'Location', 'Southeast');
    grid on;
end
