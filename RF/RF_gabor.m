% This script trains an RF classifier using the Gabor features
...
% Print some information about the resulting feature sets
fprintf('Converted training features dimension: %d x %d\n', size(trainFeatures, 1), size(trainFeatures, 2));
fprintf('Converted test features dimension: %d x %d\n', size(testFeatures, 1), size(testFeatures, 2));

%% Evaluate the optimal number of trees
numTreesRange = 10:10:150;
accuracyList = zeros(length(numTreesRange), 1);

for idx = 1:length(numTreesRange)
    numTrees = numTreesRange(idx);
    
    % Train the Random Forest classifier
    forest = TreeBagger(numTrees, trainFeatures, trainLabels, 'Method', 'classification', 'OOBPrediction', 'on');
    
    % Predict labels for the test data
    [testPredictions, ~] = predict(forest, testFeatures);
    testPredictions = str2double(testPredictions); % Convert cell to numeric
...
ylabel('Accuracy (%)');
title('Accuracy vs. Number of Trees in Random Forest (Gabor Features)');
grid on;
...
forest = TreeBagger(optimalNumTrees, trainFeatures, trainLabels, 'Method', 'classification', 'OOBPrediction', 'on');

% Predict labels for the test data with the optimal model
[testPredictions, scores] = predict(forest, testFeatures);
...
title('Confusion Matrix for Optimal Random Forest Classifier (Gabor Features)');
...
title('ROC Curve for Optimal RF Classifier (Gabor Features)');
