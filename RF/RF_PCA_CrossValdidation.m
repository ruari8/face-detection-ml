...
% Use a validation set (80-20 split)
...
%% Cross-Validate Using Optimal Number of Trees
fprintf('\nPerforming k-fold cross-validation with %d trees...\n', optimalNumTrees);
k = 5; % Number of folds
...
for fold = 1:k
    ...
    % Split data
    foldTrainFeatures = trainPCAFeatures(trainIdx, :);
    foldTrainLabels = trainLabels(trainIdx);
    foldValFeatures = trainPCAFeatures(valIdx, :);
    foldValLabels = trainLabels(valIdx);
    
    % Train model
    forest = TreeBagger(optimalNumTrees, foldTrainFeatures, foldTrainLabels, 'Method', 'classification', 'OOBPrediction', 'on');
    
    % Validate model
    [valPredictions, ~] = predict(forest, foldValFeatures);
    valPredictions = str2double(valPredictions);
    
    % Compute accuracy
    confMat = confusionmat(foldValLabels, valPredictions);
    TP = confMat(1, 1);
    FN = confMat(1, 2);
    FP = confMat(2, 1);
    TN = confMat(2, 2);
    cvAccuracy(fold) = (TP + TN) / (TP + FP + FN + TN) * 100;
end
