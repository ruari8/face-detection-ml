...
for fold = 1:k
    ...
    ndim = 36;
    [eigenVectors, eigenValues, meanX, trainPCAFeatures] = PrincipalComponentAnalysis(foldTrainImages, ndim);
    ...
    % Project validation data onto PCA space
...
% Apply PCA to all training data
[eigenVectors, eigenValues, meanX, trainPCAFeatures] = PrincipalComponentAnalysis(allImages, ndim);

% Train final model with average best parameters
finalModel = fitcsvm(trainPCAFeatures, allLabels, 'KernelFunction', 'rbf', ...
...
[predictedLabels, scores] = predict(finalModel, testPCAFeatures);
