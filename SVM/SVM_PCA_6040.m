...
fprintf('Applying PCA to training data...\n');
% Define number of PCA dimensions to keep
ndim = 36;
[eigenVectors, eigenValues, meanX, trainPCAFeatures] = PrincipalComponentAnalysis(trainImages, ndim);
...
svmModel = fitcsvm(trainPCAFeatures, trainLabels, 'KernelFunction', 'rbf', 'Standardize', true, ...
...
% Step 5: Project test data onto PCA space
fprintf('Projecting test data onto PCA space...\n');
% First center the test data using training mean
...
[predictedLabels, scores] = predict(svmModel, testPCAFeatures);
...
title('Confusion Matrix for SVM Classifier (PCA Features)');
