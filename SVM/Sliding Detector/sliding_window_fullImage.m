...
function [finalDetections, allScores] = sliding_window_fullImage(image, svmModel)
    ...
    for scaleIdx = 1:length(scales)
        ...
        for y = 1:currentStepSize:(resizedHeight - windowSize(1))
            for x = 1:currentStepSize:(resizedWidth - windowSize(2))
                ...
                % Convert window to feature vector format (raw pixels)
                features = double(reshape(window, 1, 486));  % 27*18 = 486 features
