...
fprintf('Extracting HOG features for all training data...\n');
allHOGFeatures = [];
for i = 1:size(allImages, 1)
    image = reshape(allImages(i, :), [imageHeight, imageWidth]);
    feature = hog_feature_vector(image);
    allHOGFeatures = [allHOGFeatures; feature];
end
...
for fold = 1:k
    ...
    [predictedLabels, scores] = predict(svmModel, foldValidHOG);
...
finalModel = fitcsvm(allHOGFeatures, allLabels, 'KernelFunction', 'rbf', ...
                    'BoxConstraint', mean(allBestC), ...
                    'KernelScale', mean(allBestKernelScale), ...
                    'Standardize', true);

% Evaluate on test set
[predictedLabels, scores] = predict(finalModel, testHOGFeatures);
